              ...
              <table class="table table-bordered text-nowrap">
                ...
                ';
                  $query = mysqli_query($conn, "SELECT a.*, b.name, b.price FROM tb_cart a
                                                JOIN tb_product AS b ON a.id_product = b.id_product WHERE id_customer = '". $_SESSION['id_cust'] ."' AND a.status = 'cart'");
                  while ($row = mysqli_fetch_assoc($query)) {
                    $id_cart = $row['id_cart'];
            ...
            <div class="cart-empty text-center">
              ...
              <div class="col-xxl-3">
                <div class="card custom-card">
                  ...
                  ';
                  $queryCart = mysqli_fetch_assoc(mysqli_query($conn, "SELECT SUM(biaya) AS ttl_ksl FROM tb_cart WHERE id_customer = '". $_SESSION['id_cust'] ."' AND status = 'cart'"));
                  $query = mysqli_query($conn, "SELECT * FROM tb_cart WHERE id_customer = '". $_SESSION['id_cust'] ."' AND status = 'cart'");
...
if (isset($_POST['ubah_data_qty_tambah'])) {
    $hasil = "";
    $no_id = mysqli_real_escape_string($conn, $_POST['ubah_data_qty_tambah']);
    $id_product = mysqli_real_escape_string($conn, $_POST['id_product']);
...
if (isset($_POST['del_cart'])) {
  $hasil = "";
  $id_hapus = mysqli_real_escape_string($conn, $_POST['del_cart']);
  $deleteCart = mysqli_query($conn, "DELETE FROM tb_cart WHERE id_cart = '". $id_hapus ."' AND id_customer = '". $_SESSION['id_cust'] ."'");
